(async function () {
  ...
  // ---- 3. Build the page-flip book ----
  // NOTE: we deliberately use loadFromHTML() instead of loadFromImages().
  // loadFromImages() renders the whole spread onto a single internal
  // <canvas>, which bakes StPageFlip's built-in spine/shadow gradient into
  // canvas pixels — not reachable by CSS, and not gated by the
  // drawShadow/maxShadowOpacity settings (those only affect the shadow
  // drawn *during* a page turn). loadFromHTML() instead renders each page
  // as a real DOM element. Note: StPageFlip's own drawShadow /
  // maxShadowOpacity only affect the shadow drawn *during* a flip — the
  // library does not render any shadow on a static, resting spread. The
  // static "gutter" seam is added separately below (see .gutter-shadow)
  // so it stays visible at rest and is independently tunable.
  ...
  // Build one real page element per rendered image and attach them to the
  // container BEFORE handing them to the library — loadFromHTML() expects
  // actual DOM nodes it can take over, unlike loadFromImages() which builds
  // everything internally from bare URLs.
  const pageEls = images.map(function (src) {
